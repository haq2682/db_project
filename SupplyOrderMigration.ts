    ...
    try {
        await new Promise((resolve) => {
            sql.query(`SELECT * FROM information_schema.tables WHERE table_schema = 'pharmacy' AND table_name = 'users'`, function(error, results) {
                ...
                if(results.length === 0) {
                    console.error("Users table does not exist. You must first add the users table before adding the supply orders table.");
                    return;
                }
        ...
        await new Promise((resolve) => {
            ...
            `, function(error, results) {
                if (error) {
                    console.error("Error creating supply orders table: ", error.sqlMessage);
                    return;
                }
